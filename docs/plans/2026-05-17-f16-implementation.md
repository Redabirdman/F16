...
> **⚠️ STATUS / RECONCILIATION (updated 2026-06-03).** This is the ORIGINAL 2026-05-17 plan. Milestone scope + numbering have evolved during execution — the authoritative live status is tracked in claude-mem memory `project_milestone_status.md`, not here. Key divergences so far:
>
> - **M8 (Maxance Operator)** shipped as a **Chrome MV3 extension driver** (not the Stagehand/Playwright path described below — Cloudflare blocks headless browsers; see memory `project_maxance_access.md` + `project_hosting_pivot.md`). **CLOSED + live-verified 2026-06-03**: autonomous quote.preview → quote.confirm (real devis) → emails the devis PDF, with phase-2j self-healing. Trottinette/NVEI only.
> - **M9** in this doc = "Quote Orchestration"; in live tracking M9 became **"Maxance product catalog → agent knowledge"** and is **CLOSED + live-verified 2026-06-03**: the 15 Maxance product fiches were curated into `F16/MAXANCE catalogue produits agent.md` and ingested into `knowledge_chunks` (source `maxance_product_catalog`), so the Sales Agent can discuss any product via RAG. Live auto-quoting stays **trottinette-only**; extending quoting to other products is a FUTURE (post-V1) effort, product-by-product.
> - This also fulfils the intent of **M7.T6** ("Maxance product-pages ingest") — done via curated PDF fiches instead of a live Stagehand scrape (better RAG quality, no portal dependency).
> - **M10 (voice)** built + deployed, awaiting the live call (commits `0683f34`/`2e1bc8f`/`a7ad20d`/`b81525b`). Cascaded: **OVH SIP trunk → Asterisk (native ARM64, AudioSocket) → Pipecat → Deepgram (Nova-3 STT + Aura-2-agathe-fr TTS) → shared Sales Agent brain (`reply-core.ts` + `POST /v1/voice/turn`)**. NOTE the gateway pivoted from **jambonz → Asterisk**: this PC is ARM64 and jambonz is x86-only (ran under qemu, unstable). Asterisk 20 is deployed in a WSL2 Ubuntu distro on this PC, OVH trunk **registered**; remaining = the coordinated live outbound call. See memory `project_f16_voice_infra.md` + `f16-m10-arm64-jambonz-constraint`.
> - Several later milestones (M11, M13, M14, M15) shipped ahead of / out of the strict order below. See `project_milestone_status.md` for the real table.
> - Local dev DB / ingest runbook: see memory `project_f16_dev_db.md` (Postgres on host **5435**, `--env-file=.env` for scripts).
...
_Plan saved 2026-05-17. Ridaa to pick execution mode._
