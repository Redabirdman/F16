...
> - **M10 (voice)** is in progress: the **code half is done + verified** (commit `0683f34`) — the Sales Agent brain is now reusable synchronously (`reply-core.ts` + `POST /v1/voice/turn`), and the Pipecat bridge (`pipecat/`) calls it with HMAC auth. Stack: cascaded (STT→brain→TTS), **Deepgram** for both STT+TTS (Azure fallback), **OVH SIP trunk → Jambonz gateway → Pipecat**. Remaining for a live call: real Deepgram STT/TTS wiring, the Jambonz gateway, and the live outbound test.
